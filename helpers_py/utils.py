...
"""
------------------------------------ AUTOPILOT ACTIONS ------------------------------------------
A simple convenience class to return useful info around action names available in the fact table.
-------------------------------------------------------------------------------------------------
"""
...
class AutopilotActions:
    """
    Contains relevant information for the types of actions that Autopilot's customers generate,
    ...
    def __init__(self):
        ...
        self.actions_list = []
        self.families_list = []
        self.actions_table = pd.DataFrame()

        # Get action actions_dict
        self.actions_dict = self._generate_action_dict()

        # Get a list of all actions
        self.actions_list = list(self.actions_dict.keys())

        # Define families
        self.families_list = list(set([key.split(".")[0] for key in self.actions_list])) + ["email.transactional"]

        # Generate a table for the actions
        self.actions_table = self._generate_action_table()

        ea_table = self.actions_table

        self.pressure_actions = list(ea_table[ea_table["engagement"] == "pressure"]["action"])
        self.positive_actions = list(ea_table[ea_table["engagement"] == "positive"]["action"])
        self.negative_actions = list(ea_table[ea_table["engagement"] == "negative"]["action"])

    def engagement_index(self, action):

        ea_table = self.actions_table
        if action not in ea_table["action"].values:
            raise ValueError(f"action {action} invalid. Must be one of {ea_table['action']}")

        engagement_index = ea_table[ea_table["action"] == action]["engagement_index"].iloc[0]

        return engagement_index

    def _generate_action_dict(self):

        dictionary = {'converted': 'created automatically by the API when contact meets conversion criteria for a goal',
                      'email.bounced': 'when an email to a contact is bounced',
                      'email.clicked': 'when a contact clicks on a link in an email',
                      'email.deferred': 'when an email is unable to be sent, and is scheduled to be reattempted later',
                      'email.opened': 'when a contact opens an email',
                      'email.replied': ' when a contact replies to an email',
                      'email.reported': 'when a contact reports an email as spam',
                      'email.sent': 'when an email is sent to a contact',
                      'email.unsubscribed': 'when a contact unsubscribes from an email',
                      'email.transactional.bounced': 'same as email.bounced but for transactional email',
                      'email.transactional.clicked': 'same as email.clicked but for transactional email',
                      'email.transactional.deferred': 'same as email.deferred but for transactional email',
                      'email.transactional.opened': 'same as email.opened but for transactional email',
                      'email.transactional.replied': 'same as email.replied but for transactional email',
                      'email.transactional.reported': 'same as email.reported but for transactional email',
                      'email.transactional.sent': 'same as email.sent but for transactional email',
                      'email.transactional.unsubscribed': 'same as email.unsubscribed but for transactional email',
                      'form.submitted': 'when a contact submits a form',
                      'headsup.clicked': 'when a contact clicks a call to action or link in a headsup message',
                      'headsup.dismissed': 'when a contact dismisses a headsup message',
                      'headsup.opened': 'when a contact opens a headsup message',
                      'headsup.responded': 'when a contact responds to a headsup message',
                      'headsup.shown': 'when a contact is shown a headsup message',
                      'journey.ejected': 'when a contact is ejected from a journey',
                      'journey.entered': 'when a contact enters a journey',
                      'list.added': 'when a contact is added to a list',
                      'list.removed': 'when a contact is removed from a list',
                      'postalmail.sent': 'when a contact is sent a piece of postal mail',
                      'segment.entered': 'when a contact enters a segment',
                      'segment.left': 'when a contact leaves a segment',
                      'sms.sent': 'when a contact is sent an SMS message'}
        return dictionary

    def _generate_action_table(self):
        """
        Generates table containing all autopilot actions along with
        their metadata (labels, direction etc)
        """
        actions_by_family = {}
        for family in self.families_list:
            if family == 'email':
                selected = [a for a in self.actions_list if (('email.transactional.' not in a) & ('email' in a))]
            elif family == 'email.transactional':
                selected = [a for a in self.actions_list if 'email.transactional.' in a]
            else:
                selected = [name for name in self.actions_list if family in name]
            actions_by_family[family] = selected

        order = ["email", "email.transactional", "headsup", "journey", "segment",
                 "list", "sms", "converted", "form", "postalmail"]

        table = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in actions_by_family.items()]))[order]

        table.columns.name = "family"
        table = table.stack().to_frame()
        table.index = table.index.droplevel()
        table = table.rename(columns={0: "action"}).reset_index()
        table["engagement"] = np.nan
        table["direction"] = np.nan

        eng_labels = AutopilotActions.engagement_labels()
        for label, this_actions in eng_labels.items():
            where = table["action"].isin(this_actions)
            table.loc[where, "engagement"] = label

        dir_labels = AutopilotActions.direction_labels()
        for label, this_actions in dir_labels.items():
            where = table["action"].isin(this_actions)
            table.loc[where, "direction"] = label

        table["engagement_index"] = table["engagement"].replace({"positive": 1, "pressure": 1, "negative": -1})
        table = table.sort_values(by=["family", "engagement_index"]).reset_index(drop=True)

        return table

    def family_validate(self, family):
        """
        Raises an error if family is not in self.families_list
        Parameters
        ----------
        family :    str
        """
        ...
        # Extract first part of action
        if family not in self.families_list:
            raise ValueError(f"family '{family}' invalid. Valid families are {self.families_list}")
        else:
            return True

    def actions_select(self, family=None, engagement=None, direction=None, engagement_index=None):
        """
        Returns a list with the names of the actions that match the string family.
        Before doing anything, it checks that family is a valid string.
        """

        # Get actions for this family
        df = self.actions_table

        actions = df_select(df=df, family=family, engagement=engagement,
                            direction=direction, engagement_index=engagement_index)["action"].values

        return actions

    @staticmethod
    def engagement_labels():

        labels = {"pressure": ['email.sent', 'email.transactional.sent',
                                'headsup.shown', 'sms.sent', 'postalmail.sent'],
                  "positive": ['email.clicked', 'email.transactional.clicked',
                                'email.opened', 'email.transactional.opened',
                                'email.replied', 'email.transactional.replied',
                                'form.submitted', 'headsup.responded',
                                'headsup.opened', 'headsup.clicked'],
                  "negative": ['email.reported', 'email.transactional.reported',
                                'email.unsubscribed', 'email.transactional.unsubscribed',
                                'headsup.dismissed']}
    ...
    def direction_labels():
        ...
        labels = {"to_user": ['email.sent', 'email.transactional.sent',
                              'headsup.shown', 'sms.sent', 'postalmail.sent'],
                 "from_user": ['headsup.clicked',
                               'form.submitted',
                               'email.clicked',
                               'email.transactional.clicked',
                               'headsup.dismissed',
                               'headsup.opened',
                               'email.opened',
                               'email.transactional.opened',
                               'headsup.responded',
                               'email.replied',
                               'email.transactional.replied',
                               'email.reported',
                               'email.transactional.reported',
                               'email.unsubscribed',
                               'email.transactional.unsubscribed'],
                 "from_system": ['email.bounced', 'email.transactional.bounced',
                                 'journey.ejected', 'segment.entered', 'list.added',
                                 'converted', 'journey.entered', 'list.removed',
                                 'segment.left', 'email.deferred', 'email.transactional.deferred'
                                 ]}
        return labels
